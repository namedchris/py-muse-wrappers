import csv
from typing import Any, Dict, Callable, Optional
from dataclasses import dataclass
from component import Component
from buttons import BaseButton


@dataclass
class PanelConfig:
    """Configuration for a panel loaded from CSV"""
    name: str
    pages: Dict[str, 'PageConfig']


@dataclass
class PageConfig:
    """Configuration for a page"""
    name: str
    buttons: Dict[str, 'ButtonConfig']


@dataclass
class ButtonConfig:
    """Configuration for a button from CSV"""
    name: str
    channel_port: int
    channel_code: int
    address_port: int
    address_code: int
    level_port: Optional[int] = None
    level_code: Optional[int] = None


class Panel(Component):
    """Panel component - top level container for pages"""

    def __init__(self, muse_device: Any, config: PanelConfig):
        """
        Initialize panel with MUSE device and configuration.

        Args:
            muse_device: The py4j device from context.devices.get()
            config: Panel configuration loaded from CSV
        """
        super().__init__(config.name)
        self._device = muse_device
        self.config = config

        # Initialize all pages from config
        for page_name, page_config in config.pages.items():
            page = Page(self, page_config)
            self.add_subcomponent(page)

    def send_command(self, port: int, cmd: str):
        """
        Send NetLinx command string to device on specified port.

        Args:
            port: Port number (1-indexed as in NetLinx)
            cmd: Command string to send
        """
        self._device.port[port - 1].send_command(cmd)

    def get_page(self, name: str) -> Optional['Page']:
        """Get page by name (searches subcomponents list)"""
        for page in self.subcomponents:
            if page.name == name:
                return page
        return None

    def render(self):
        """Render panel (no panel-level rendering needed)"""
        pass


class Page(Component):
    """Page component - contains buttons"""

    def __init__(self, panel: Panel, config: PageConfig):
        """
        Initialize page with panel reference and configuration.

        Args:
            panel: Parent panel
            config: Page configuration
        """
        super().__init__(config.name)
        self.panel = panel
        self.config = config

        # Initialize all buttons from config
        for button_key, button_config in config.buttons.items():
            # Create a closure that captures the port for this button
            port = button_config.channel_port
            send_cmd = lambda cmd, p=port: panel.send_command(p, cmd)

            button = BaseButton(
                name=button_key,
                port=button_config.channel_port,
                address=button_config.address_code,
                channel=button_config.channel_code,
                send_command=send_cmd
            )
            self.add_subcomponent(button)

    def get_button(self, name: str) -> Optional[BaseButton]:
        """Get button by name (searches subcomponents list)"""
        for button in self.subcomponents:
            if button.name == name:
                return button
        return None

    def render(self):
        """Render page (no page-level rendering needed)"""
        pass


def parse_panel_csv(csv_path: str) -> PanelConfig:
    """
    Parse panel configuration CSV file.

    Args:
        csv_path: Path to CSV file

    Returns:
        PanelConfig with all pages and buttons
    """
    with open(csv_path, 'r') as f:
        lines = f.readlines()

    # Get panel name from first line
    panel_name = lines[0].strip().strip('"')

    # Find the function codes section
    function_section_start = None
    for i, line in enumerate(lines):
        if "Function codes utilized:" in line:
            function_section_start = i + 3  # Skip blank line and header
            break

    if function_section_start is None:
        raise ValueError("Could not find function codes section in CSV")

    # Parse CSV data starting from function codes
    reader = csv.reader(lines[function_section_start:])

    pages: Dict[str, PageConfig] = {}
    current_page_name = None

    for row in reader:
        if not row or not any(row):  # Empty row
            break

        # Check if this is a page header (only has one column with content)
        if len(row) == 1 and row[0].strip():
            current_page_name = row[0].strip().strip('"')
            if current_page_name not in pages:
                pages[current_page_name] = PageConfig(
                    name=current_page_name,
                    buttons={}
                )
            continue

        # This is a button row
        if current_page_name and len(row) >= 4:
            # Skip if button name is empty
            if not row[1].strip():
                continue

            button_name = row[1].strip().strip('"')

            # Parse channel (format: "port:code") - strip spaces
            channel_str = row[2].strip()
            if not channel_str:
                continue
            channel_parts = channel_str.split(':')
            channel_port = int(channel_parts[0].strip())
            channel_code = int(channel_parts[1].strip())

            # Parse address (format: "port:code") - strip spaces
            address_str = row[3].strip()
            if not address_str:
                continue
            address_parts = address_str.split(':')
            address_port = int(address_parts[0].strip())
            address_code = int(address_parts[1].strip())

            # Parse level if present
            level_port = None
            level_code = None
            if len(row) > 4 and row[4].strip():
                level_parts = row[4].strip().split(':')
                level_port = int(level_parts[0].strip())
                level_code = int(level_parts[1].strip())

            button_config = ButtonConfig(
                name=button_name,
                channel_port=channel_port,
                channel_code=channel_code,
                address_port=address_port,
                address_code=address_code,
                level_port=level_port,
                level_code=level_code
            )

            # Use button name + codes as key to handle duplicate names
            button_key = f"{button_name}_{address_port}_{address_code}"
            pages[current_page_name].buttons[button_key] = button_config

    return PanelConfig(name=panel_name, pages=pages)


def load_panel(muse_device: Any, csv_path: str) -> Panel:
    """
    Load panel from CSV configuration and MUSE device.

    Args:
        muse_device: Device from context.devices.get("AMX-10001")
        csv_path: Path to panel CSV configuration file

    Returns:
        Configured Panel instance
    """
    config = parse_panel_csv(csv_path)
    return Panel(muse_device, config)
